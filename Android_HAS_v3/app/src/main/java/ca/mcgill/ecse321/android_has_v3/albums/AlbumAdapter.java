...
public class AlbumAdapter extends BaseAdapter{
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        View vi = convertView;
        if(vi == null)
            vi = inflater.inflate(R.layout.albums_listview_item, null);

        Album album = albums.get(position);

        TextView albumName = (TextView) vi.findViewById(R.id.album_nameTextView);
        albumName.setText(album.getName());

        //Formats date as "DD-MM-YYYY"
        TextView releaseDate = (TextView) vi.findViewById(R.id.album_releaseTextView);
        Calendar c = Calendar.getInstance();
        c.setTime(album.getReleaseDate());
        releaseDate.setText(String.format("%02d-%02d-%04d", c.get(Calendar.DAY_OF_MONTH),
                c.get(Calendar.MONTH) + 1, c.get(Calendar.YEAR)));

        TextView mainArtist = (TextView) vi.findViewById(R.id.album_artistTextView);
        mainArtist.setText(album.getMainArtist().getName());

        TextView genre = (TextView) vi.findViewById(R.id.album_genreTextView);
